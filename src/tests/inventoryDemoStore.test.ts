...
describe("stock opname", () => {
  it("snapshots the system quantity when the sheet opens", () => {
    const before = demo.qtyOnHand(WSK, UTAMA);
    const opname = demo.startOpname(UTAMA);

    // A sale after the snapshot must not change what the count is compared
    // against — otherwise the variance measures the count, not the shortage.
    demo.postAdjustment({
      operation: "adjustment",
      productId: WSK,
      warehouseId: UTAMA,
    ...
    const item = demo.opnameItemsOf(opname._id).find((i) => i.productId === WSK)!;
    expect(item.systemQty).toBe(before);
    expect(demo.qtyOnHand(WSK, UTAMA)).not.toBe(before);
  });

  it("writes nothing for a line left blank", () => {
    const opname = demo.startOpname(UTAMA);

    const written = demo.submitOpname(opname._id);
    expect(written).toHaveLength(0);
  });

  it("writes nothing for a line counted equal to the system", () => {
    const opname = demo.startOpname(UTAMA);
    const item = demo.opnameItemsOf(opname._id).find((i) => i.productId === WSK)!;

    demo.setOpnameCount(item._id, item.systemQty);
    const written = demo.submitOpname(opname._id);

    // A movement of zero is a row with no meaning that every report must skip.
    expect(written).toHaveLength(0);
  });

  it("writes one movement per line that differs", () => {
    const opname = demo.startOpname(UTAMA);
    const items = demo.opnameItemsOf(opname._id);
    const wsk = items.find((i) => i.productId === WSK)!;
    const shampoo = items.find((i) => i.productId === SHAMPOO)!;

    demo.setOpnameCount(wsk._id, "5");
    demo.setOpnameCount(shampoo._id, "20");

    const written = demo.submitOpname(opname._id);

    expect(written).toHaveLength(2);
    expect(written.every((m) => m.movementType === "opname_diff")).toBe(true);
    expect(written[0].reference).toEqual({ type: "stock_opname", id: opname._id });
  });

  it("moves the balance by the counted difference", () => {
    const opname = demo.startOpname(UTAMA);
    const item = demo.opnameItemsOf(opname._id).find((i) => i.productId === SHAMPOO)!;

    demo.setOpnameCount(item._id, "20");
    demo.submitOpname(opname._id);

    expect(demo.qtyOnHand(SHAMPOO, UTAMA)).toBe("20.0000");
  });

  it("values the variance at the SNAPSHOTTED cost, not today's", () => {
    const opname = demo.startOpname(UTAMA);
    const item = demo.opnameItemsOf(opname._id).find((i) => i.productId === SHAMPOO)!;

    // Shampoo's average was 44000 when the sheet opened. A receipt at a higher
    // price afterwards must not restate a discrepancy measured before it.
    demo.postAdjustment({
      operation: "adjustment",
      productId: SHAMPOO,
      warehouseId: UTAMA,
    ...
    demo.setOpnameCount(
      item._id,
      toDecimalString((toMinor(item.systemQty) ?? 0n) - toMinor("2")!),
    );

    // Re-read: setOpnameCount replaces the row, so the captured object is stale.
    const counted = demo.opnameItemsOf(opname._id).find((i) => i._id === item._id)!;
    const diff = demo.opnameDiff(counted)!;

    // 2 short × the 44,000 average snapshotted at open — NOT the 90,000 the
    // receipt above pushed it to.
    expect(toDecimalString(diff.value)).toBe("-88000.0000");
  });

  it("refuses to submit the same sheet twice", () => {
    const opname = demo.startOpname(UTAMA);
    const item = demo.opnameItemsOf(opname._id)[0];
    demo.setOpnameCount(item._id, "1");
    demo.submitOpname(opname._id);

    expect(() => demo.submitOpname(opname._id)).toThrow(/sudah final/i);
  });
});
