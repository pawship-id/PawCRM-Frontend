...
/**
 * CETAK FAKTUR — the customer's own copy, on A4.
 *
 ...
 */
describe("InvoiceSheet — the customer's copy of the bill", () => {
  async function openPrint(invoice = paidDetail()) {
    const user = userEvent.setup();
    asMock(customerInvoiceService.getById).mockResolvedValue(invoice);

    renderWithAuth(<InvoiceDetail invoiceId={INVOICE_ID} />);

    await user.click(await screen.findByRole("button", { name: /^Cetak$/ }));
    return within(await screen.findByRole("dialog"));
  }

  it("heads the sheet FAKTUR, with the number and the shop", async () => {
    const dialog = await openPrint();

    expect(dialog.getByText("FAKTUR")).toBeInTheDocument();
    // Twice: the letterhead, and again over the signature line.
    expect(dialog.getAllByText("Buloo Petshop").length).toBe(2);
    expect(dialog.getAllByText("INV-2026-0042").length).toBeGreaterThan(0);
  });

  it("names who it is billed to", async () => {
    const dialog = await openPrint();

    expect(dialog.getByText("Ditagihkan kepada")).toBeInTheDocument();
    expect(dialog.getAllByText("Bu Sari").length).toBeGreaterThan(0);
  });

  /*
    THE WHOLE BILL, NOT ONE PAYMENT — the line that separates this document from
    the kwitansi. Both figures belong here: what the invoice came to, and what is
    still owed after the payments listed above them.
  */
  it("shows the transaction value and what is still owed", async () => {
    const dialog = await openPrint();

    expect(dialog.getByText("Nilai Transaksi")).toBeInTheDocument();
    expect(dialog.getByText("SISA TAGIHAN")).toBeInTheDocument();
  });

  /*
    EACH PAYMENT ON ITS OWN LINE. A customer reconciling against a bank statement
    is matching individual transfers; one summed "Dibayar" sends them back to the
    counter to ask which ones it covered.
  */
  it("lists the payments individually", async () => {
    const dialog = await openPrint();

    expect(dialog.getByText(/^Dibayar /)).toBeInTheDocument();
  });

  /*
    A SHORT DATE ON THE PAYMENT LINES, and the header keeps the long one.

    The totals column is one narrow block shared by a label and a figure. With
    "30 Agustus 2026 (Mandiri)" in it, what wrapped was the AMOUNT — "−Rp" on one
    line and "45.250" on the next, which is not a number anybody can read down a
    column.
  */
  it("dates a payment line short, so the figure beside it fits", async () => {
    const dialog = await openPrint();

    expect(dialog.getByText(/^Dibayar 27 Agu 2026/)).toBeInTheDocument();
    expect(dialog.queryByText(/^Dibayar 27 Agustus/)).not.toBeInTheDocument();
  });
  ...
  */
  it("leaves a cancelled payment off the sheet", async () => {
    const dialog = await openPrint(
      paidDetail([paymentRow({ isVoided: true, voidedAt: "2026-08-28T00:00:00.000Z" })]),
    );

    expect(dialog.queryByText(/^Dibayar /)).not.toBeInTheDocument();
  });

  /*
    THE WORST THING THIS SHEET COULD PRODUCE is a voided invoice that looks
    valid — a document somebody can be asked to pay against. Said first and
    unmissably, and the outstanding line goes away with it.
  */
  it("stamps a voided invoice, and stops asking for money", async () => {
    ...
    expect(dialog.getByText(/FAKTUR INI DIBATALKAN/)).toBeInTheDocument();
    expect(dialog.getByText(/Salah pelanggan/)).toBeInTheDocument();
    expect(dialog.queryByText("SISA TAGIHAN")).not.toBeInTheDocument();
  });
  ...
  */
  /*
    THE SHOP'S OWN FOOTER — usually where to send the money. An invoice that
    bills somebody without saying that makes them ring up to ask.
  */
  describe("the shop's footer note", () => {
    const withNote = (invoiceFooterNote: string) =>
      asMock(tenantService.me).mockResolvedValue({
        _id: "t1",
        name: "Buloo Petshop",
    ...
    it("prints what the shop wrote, line breaks kept", async () => {
      withNote("Bayar ke BCA 1234567890\na.n. Buloo Petshop");
      const dialog = await openPrint();

      expect(
        dialog.getByText(/Bayar ke BCA 1234567890/),
      ).toBeInTheDocument();
    });

    /*
      NOTHING AT ALL WHEN EMPTY — no heading, no blank block. A shop that takes
      every payment at the counter has nothing to put here, and a stray label on
      a customer's copy is worse than a shorter page.
    */
    it("prints nothing when the shop set none", async () => {
      const dialog = await openPrint();

      expect(dialog.queryByText(/Bayar ke BCA/)).not.toBeInTheDocument();
    });

    /*
      NOT ON A VOIDED INVOICE. "Pembayaran ditujukan ke …" on a bill that has
      been cancelled invites exactly the payment the void exists to stop.
    */
    it("keeps it off a cancelled invoice", async () => {
      withNote("Bayar ke BCA 1234567890");
      const dialog = await openPrint(detail({ status: "void" }));
  ...
  it("prints a pre-PCR-030 invoice with the little it has", async () => {
    const dialog = await openPrint(detail({ items: [], totals: null }));

    expect(dialog.getByText("FAKTUR")).toBeInTheDocument();
    expect(dialog.getByText("Nilai Transaksi")).toBeInTheDocument();
    // No line table to draw, so no header for one.
    expect(dialog.queryByText("Deskripsi")).not.toBeInTheDocument();
  });
});
