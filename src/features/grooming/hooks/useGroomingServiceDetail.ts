...
/**
 * The tahapan other services of the same line already use — what "+ Tambah
 * tahapan…" offers, most used first.
 *
 * THERE IS NO SHOP-WIDE LIST OF TAHAPAN. The mockup picks from one; here a
 * tahapan is free text on each service, so the nearest honest list is the names
 * already in use, and a new name can still be typed. Capped at the API's page of
 * 100 services. `businessLineId` null asks nothing — a role that cannot change
 * the tahapan has no use for suggestions.
 *
 * FAILS SOFTLY: without suggestions a name can still be typed.
 */
export function useLineSessionNames(
  businessLineId: string | null,
  excludeServiceId: string,
) {
  const key = businessLineId ?? "";
  const [loaded, setLoaded] = useState<{
    key: string;
    names: string[];
    failed: boolean;
  }>({ key: "", names: [], failed: false });
  ...
  useEffect(() => {
    ...
    serviceService
      .list({ businessLineId: key, limit: 100 })
      .then((result) => {
        if (!active) return;

        const seen = new Map<string, { name: string; count: number }>();
        for (const other of result.items) {
          if (other._id === excludeServiceId) continue;
          for (const session of other.sessions ?? []) {
            const name = session.trim();
            if (name === "") continue;

            const entry = seen.get(name.toLowerCase());
            if (entry) entry.count += 1;
            else seen.set(name.toLowerCase(), { name, count: 1 });
          }
        }
      ...
      .catch(() => {
        if (active) setLoaded({ key, names: [], failed: true });
      });

    return () => {
      active = false;
    };
  }, [key, excludeServiceId]);

  if (key === "") return { names: [] as string[], loading: false, failed: false };

  const current = loaded.key === key;
  return {
    names: current ? loaded.names : [],
    loading: !current,
