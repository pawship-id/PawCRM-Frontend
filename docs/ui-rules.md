...
**Module mapping:** Produk & Varian (6, incl. sort), Kategori (3, incl. sort), Batch & Expired (4, incl. sort), Stok Opname (4, incl. sort), Supplier (4, incl. sort), Penerimaan Barang (5, incl. sort) and Retur ke Supplier (5, incl. sort) → filter panel at every width, one `Filter (n)` button beside search · Utang Supplier (3, incl. sort) → the same panel, with the view lens as a pill row outside it · Kartu Stok (4, incl. sort) → filter panel beside search; Gudang and Produk stay outside it as required inputs, not filters · Sales / Invoice / Booking when built (~8) → filter panel.
...
- 3 screens with filters written inline rather than in a toolbar — `JournalEntriesScreen`, `StockOnHandScreen`, `ProductDetail` — still carry their own `const ALL = "all"` sentinel and a raw `ui/select`. They were not part of the 15-toolbar census; migrate them to `@/components` filters. (`ChartOfAccountsScreen` is done.) `JournalEntriesScreen` also still reads `features/accounting/data/dummy.ts`, so its migration and its wiring to `GET /api/journal-entries` are the same piece of work — `FinanceDashboardToolbar` is the shape to copy.
