...
**Module mapping:** Produk & Varian (6, incl. sort), Kategori (3, incl. sort) and Batch & Expired (4, incl. sort) → filter panel at every width, one `Filter (n)` button beside search · Kartu Stok (4) → quick bar, Produk select is searchable · Supplier (2) → quick bar, tipe as pills · Sales / Invoice / Booking when built (~8) → filter panel.
