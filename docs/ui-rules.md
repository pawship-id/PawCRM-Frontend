...
**Module mapping:** Produk & Varian (6, incl. sort), Kategori (3, incl. sort), Batch & Expired (4, incl. sort) and Stok Opname (4, incl. sort) → filter panel at every width, one `Filter (n)` button beside search · Kartu Stok (4, incl. sort) → filter panel beside search; Gudang and Produk stay outside it as required inputs, not filters · Supplier (2) → quick bar, tipe as pills · Sales / Invoice / Booking when built (~8) → filter panel.
