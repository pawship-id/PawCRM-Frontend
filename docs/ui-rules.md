...
**Module mapping:** Produk & Varian (6, incl. sort), Kategori (3, incl. sort), Batch & Expired (4, incl. sort), Stok Opname (4, incl. sort) and Supplier (3) → filter panel at every width, one `Filter (n)` button beside search · Kartu Stok (4, incl. sort) → filter panel beside search; Gudang and Produk stay outside it as required inputs, not filters · Sales / Invoice / Booking when built (~8) → filter panel.

Supplier is the one entry here decided by viewport rather than by field count: three single-selects fit a quick bar on a laptop and wrap onto three lines on a phone, and the collapse below would have meant two trees to keep in step. One panel at every width is one tree — the same reasoning that dropped the media query from Produk & Varian.
